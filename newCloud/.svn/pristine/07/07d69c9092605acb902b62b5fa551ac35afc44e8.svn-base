...
$(function() {
	...
	// 保存按钮，click事件
	$(".doSaveBuyLimitBtn").unbind("click");
	$(".doSaveBuyLimitBtn").click(function() {
		$("#buyLimitForm").submit();
	});

	$("#defaultUrl").text(defaultSaleConfig.buyLimitUrl);
	if (saleConfig != null) {
		$("#url").val(saleConfig.buyLimitUrl);
		url = saleConfig.buyLimitUrl;
	}

});

$("#buyLimitForm").validate({
