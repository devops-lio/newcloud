...
$(function() {
		...
		$('.btn-sureAdd').click(function() {
			...
			//输入框必填项为空报错
		function inputEmpty() {
			if($(".charge-design").val()==""||$(".charge-design").val()==undefined){
			...
			if($("#priceNum").val()==0||$("#priceNum").val()==''||$("#priceNum").val()==undefined){
				$("#priceNum").addClass('f');
			...
			if($('.yesOrNo').hasClass('on')) {//套餐开启
				if($('.charge-package').val() == ''||$('.charge-package').val() == 0) {
		...
		function showNewCharge(state) {
			...
			if($('.cn-input').hasClass('f')) {
				...
				$(".new-billing").css("display", "none").animate({'right':'-537px'});
		...
		function cancleBtn() {
			...
			$('.btbtn-cancle').click(function() {
				$('.btn-tipBox').css('display', 'none');
				$(".new-billing").css("display", "block").animate({'right':'0px'});
		...
		$(".shi").click(function() {
			...
			$(".liu").removeClass("on");
			$('.ct-chargeStyle').text('时长');
			$('.bs-unit').html('<option value="时">时</option>' +
				'<option value="天">天</option>' +
				'<option value="月">月</option>')
			$('.bs-unit1').html('<option value="时">时</option>' +
				'<option value="天">天</option>' +
				'<option value="月">月</option>')
		})
		/*流量选项*/
		$(".liu").click(function() {
			$('.shi').find('img').attr('src', imgPath+'/sele2.png');
			$('.liu').find('img').attr('src', imgPath+'/sele1.png');
			$(".liu").addClass("on");
			$(".shi").removeClass("on");
			$('.ct-chargeStyle').text('流量');
			$('.bs-unit').html('<option value="G">G</option>' +
				'<option value="M">M</option>')
			$('.bs-unit1').html('<option value="G">G</option>' +
				'<option value="M">M</option>')
		})
		...
		$('.addCodeBtn').click(function() {
				...
				for (var i = 0; i < len; i++) {
					...
					for (var j = 0; j < len; j++) {
						if(i!=j){
							if($(".package-num").eq(i).val()==$(".package-num").eq(j).val()){
		...
		success:function(data){
			...
			if(data.code==200){
				for (var i = 0; i < data.data.length; i++) {
					...
					var imgpaths = imgPath+'/'+(data.data[i].site_type.split(',')[0].split('.')[0]==''?'xuexiao':data.data[i].site_type.split(',')[0].split('.')[0])+'s.png';
					var ismain = imgPath+'/'+(data.data[i].status==900?"place-shou.png":"place-mian.png");
					htmls+='<li value="'+data.data[i].id+'">'+
							'<p class="cm-listPic"><img src="'+imgpaths+'"><span class="sm-pic"><img src="'+ismain+'"></span></p >'+
							'<p class="cm-listName">'+data.data[i].site_name+'</p >'+
							'<p class="cm-listAddress">地址 : <span>'+data.data[i].address+'</span></p >'+
							'<p class="'+(data.data[i].status==900?'charge-place':'charge-place on')+'">'+(data.data[i].status==900?'收费场所':'免费场所')+'</p >'+
							'<p class="cm-num">场所下套餐数量 : <span>'+data.data[i].nums+'</span></p >'+
							'<p class="list-msg"><img src="'+imgPath+'/charge1.png" /></p >'+
							'<p class="addPlaceBtn"><span value='+data.data[i].router_type+'>+</span>新增计费</p >'+
							'<p imgpaths='+ismain+' class="'+(data.data[i].status==900?'placeChange':'placeChange on')+'">'+(data.data[i].status==900?'设为免费场所':'设为收费场所')+'</p >'+
							'<div class="show-pStyle">场所类型：<span class="place-style">'+data.data[i].site_type.split(",")[1]+'</span></div>'+
							'</li>';
				...
				$(".newSite").click(function(){
					window.location.href=ctx+"/CloudSiteManage/index?fromTo=1";
				});
...
function getPageCount(){
	...
	$.ajax({
		...
		success:function(data){
			...
			$(".page_cont i").eq(1).text(data.totoalNum==0?0:data.totoalNum);
		}
...
function make(){
	...
	$(".list-msg").click(function() {
		...
		$(".list-msg").eq(n).find('img').attr('src', imgPath+'/user-down.png');
		$(".user_list").prepend($('.changeManage .cm-list > li').eq(n).clone());
		getMeal($(".list-msg").eq(n).parent().attr("value"));
		
		
		$(".user_list > li").unbind('click');
		$(".user_list > li").click(function() {
			$(".list-msg").eq(n).find('img').attr('src', imgPath+'/charge1.png');
			$(".mask").css("display", "none");
			$(".opacity").css("display", "none");
			...
			if(btnText == '确认修改') {
				...
				if($('.yesOrNo').hasClass('on')) {
					...
					$('.vendor-style').eq(sIndex).text(charge7);
						//厂商类型
					if(charge7 == '中国电信') {
						$('.ul_single').eq(sIndex).find('img').attr('src', imgPath+'/place6.png');
					} else if(charge7 == '中国移动') {
						$('.ul_single').eq(sIndex).find('img').attr('src', imgPath+'/place6_06.png');
					} else if(charge7 == '中国联通') {
						$('.ul_single').eq(sIndex).find('img').attr('src', imgPath+'/place6_08.png');
					}
					updateChargePrice();
	...
	$(".addPlaceBtn").click(function() {
		...
		var siteId=$(".addPlaceBtn").eq(n).parent().attr("value");
			$.ajax({
				type:"post",
				...
				success:function(data){
					eval("data="+data);
					if(data.code==200){
						if($(".addPlaceBtn span").eq(n).attr('value')=="ikuai"){
						...
						$(".bs-time").addClass("on");
						$(".charge-design").attr("disabled",false);
						var mIndex = $(".addPlaceBtn").index(this);
						$('.sureAdd-btn').attr('data-index',mIndex);
						$('.package-cont').css('display', 'none');
						$(".mask").css("display", "block");
						$(".opacity").css("background", "#000");
						$(".opacity").css("display", "block");
						$(".new-billing").css("display", "block").animate({'right':'0px'});
						$('.bs-item').find('img').attr('src', imgPath+'/sele2.png').eq(0).attr('src',imgPath+'/sele1.png');
						$('.bs-unit').html('<option value="时">时</option>' +
							'<option value="天">天</option>' +
							'<option value="月">月</option>')
						$('.bs-unit1').html('<option value="时">时</option>' +
...
//		})
		...
		$('.ulPic').eq(rIndex).find('img').attr("src", imgPath+"/tuijian.png");
...
function getMeal(siteId){
	$.ajax({
		...
		success:function(data){
			...
			if(data.code==200){
				for (var i = 0; i < data.data.length; i++) {
					htmls+='<li value="'+data.data[i].priceNum+'" gNume='+data.data[i].giveNum+' gType='+data.data[i].giveType+' siteId='+data.data[i].siteId+' comder='+data.data[i].comder+' remod='+data.data[i].recommend+'  priceType='+data.data[i].chargeType+'>'+
							'<p class="ulPic"><img src="'+imgPath+"/"+(data.data[i].recommend==0?(data.data[i].isStoped!=0?"tingyong.png":"qiyong.png"):"tuijian.png")+'" /></p>'+
							'<p class="charge-sty"><span>资费名称 :<i class="cn-name">'+data.data[i].chargeName+'</i></span><span>收费价格 :<i class="cn-price">'+parseFloat(data.data[i].commPrice).toFixed2(2)+'</i>元</span></p>';
							if(data.data[i].fusePrice==undefined){
								htmls+='<p class="ul_long"><span>资费类型 :<i class="cs-name">'+data.data[i].comboTpe+'</i></span><span class="taocan"><i class="fusion-num"></i></span></p>';
							}else{
								htmls+='<p class="ul_long"><span>资费类型 :<i class="cs-name">'+data.data[i].comboTpe+'</i></span><span class="taocan">融合套餐 :<i class="fusion-num">'+parseFloat(data.data[i].fusePrice).toFixed2(2)+'</i>元</span></p>';
								
							}
							if(data.data[i].groupType==1){
								htmls+='<p class="ul_single"><span><img src="'+imgPath+'/place6.png" alt="" /><i class="vendor-style">中国电信</i></span></p>';
							}else if(data.data[i].groupType==2){
								htmls+='<p class="ul_single"><span><img src="'+imgPath+'/place6_06.png" alt="" /><i class="vendor-style">中国移动</i></span></p>';
							}else if(data.data[i].groupType==3){
								htmls+='<p class="ul_single"><span><img src="'+imgPath+'/place6_08.png" alt="" /><i class="vendor-style">中国联通</i></span></p>';
							}else{
								htmls+='<p class="ul_single"><span><img src="" alt="" /><i class="vendor-style"></i></span></p>';
							}
							htmls+='<p class="on-line"><span class="ci-name">资费说明 :</span><span class="charge-inst">'+data.data[i].describe+'</span></p>';
							if(data.data[i].isStoped==0){
								htmls+='<p class="ul_play"><button class="cm-edit">编 辑</button><button class="cm-disable">停 用</button><button class="set-recom">'+(data.data[i].recommend==0?"设为推荐":"取消推荐")+'</button></p>'+
										'</li>';
							}else{
								htmls+='<p class="ul_play"><button class="cm-edit">编 辑</button><button class="cm-disable">启 用</button><button class="set-recom" style="display:none">'+(data.data[i].recommend==0?"设为推荐":"取消推荐")+'</button></p>'+
								'</li>';
							}		
				...
				for (var i = 0; i < $(".ul_cont li").length; i++) {
					if($(".ul_cont li").eq(i).attr("remod")==1){
						if(i!=0)
							$(".ul_cont li").eq(0).before($(".ul_cont li").eq(i));
							break;
					}
				}
		...
		success:function(data){
			...
			if(data.code==200){
				if($('.placeChange').eq(n).text() == "设为免费场所") {
					...
					$(".cm-listPic").eq(n).find('.sm-pic').find('img').attr("src", imgPath+'/'+"place-mian.png")
				} else if($('.placeChange').eq(n).text() == "设为收费场所") {
					...
					$(".cm-listPic").eq(n).find('.sm-pic').find('img').attr("src", imgPath+'/'+"place-shou.png")
		...
		success:function(data){
			...
			if(data.code==200){
				if(state==1){
					...
					$('.ulPic').eq(n).find('img').attr("src", imgPath+"/tingyong.png");
				}else{
					$('.cm-disable').eq(n).text("停 用");
					$('.set-recom').eq(n).css("display", "inline-block");
					$('.ulPic').eq(n).find('img').attr("src", imgPath+"/qiyong.png");
				}
			}
			if(data.code==201){
				msg(data.msg,false);
...
function editDetail(n,str) {
	if(str=='xg'){
		...
		var cont7 =$('.cs-name').eq(n).parent().parent().parent().attr("value");//计费数量
		var cont8 =$('.cs-name').eq(n).parent().parent().parent().attr("gnume");//赠送数量
		var cont9 =$('.cs-name').eq(n).parent().parent().parent().attr("gtype");//赠送单位
		var cont10 =$('.cs-name').eq(n).parent().parent().parent().attr("comder");//套餐号段
		var cont11=$('.cs-name').eq(n).parent().parent().parent().attr("siteId");//场所id
		var cont12=$('.cs-name').eq(n).parent().parent().parent().attr("pricetype");//套餐单位
		...
		if(cont6 == '时长') {
			...
			$(".liu").css("display","none");
			$(".shi").css("display","block");
			$('.bs-unit').html('<option value='+(cont12==0?"时":cont12==1?"天":"月")+'>'+(cont12==0?"时":cont12==1?"天":"月")+'</option>')
			var ls='<option value="'+cont9+'">'+(cont9==0?"时":cont9==1?"天":"月")+'</option>';
			if(cont9==0){
			...
			$('.bs-unit').html('<option value="'+(cont12==4?"M":"G")+'">'+(cont12==4?"M":"G")+'</option>')
			var flowLs='<option value="'+cont9+'">'+(cont9==4?"M":"G")+'</option>';
			if(cont9==4){
		...
		if(!cont3 == '') { //融合套餐开启
			...
			if(cont10!=""&&cont10!=undefined){
				for (var i = 0; i < cont10.split(";").length-1; i++) {
					htmls+= '<span class="code-detail"><input type="text" maxlength="7" placeholder="如:138211" class="package-num" id="" value='+cont10.split(";")[i]+' onkeyup="this.value=this.value.replace(/\\D/g,\'\')"/><span class="dele-code">X</span></span>';
				}
				$('.code-detail').remove();
...
function updateChargePrice(){ 
	...
	var noChargePrice=$('.ct-common').text().replace(/[\u4e00-\u9fa5]+/g, '').trim();//收费单价
	var chargePrice=$('.ct-charge').text().replace(/[\u4e00-\u9fa5]+/g, '').trim();//套餐收费
	var priceNum=$('.ct-chargeNum').text().replace(/[^0-9]+/g, '').trim();//计费数量
	var priceUnit=$('.ct-chargeNum').text().charAt($('.ct-chargeNum').text().length - 1);//计费单位
	var giveNum=$(".ct-preferNum").text().replace(/[^0-9]+/g, '').trim();//优惠数量
	var giveUnit=$(".ct-preferNum").text().charAt($('.ct-preferNum').text().length - 1);//优惠单位
	var describe=$(".ct-chargeIntro").text()=="无"?"":$(".ct-chargeIntro").text();//资费说明
	var chargeGroup=$(".tc-style").text();//归属集团
	var siteId=$('.sureAdd-btn').attr("value");//场所id
	var comboNumber="";
	for (var int = 0; int < $(".package-num").length; int++) {
		if(comboNumber.indexOf($(".package-num").eq(int).val())<0&&$(".package-num").eq(int).val()!=""&$(".package-num").eq(int).val()!=undefined){
