...
public class SettlementRatioService {
	...
	public Map getYesterdayAndToday(int siteId, String agentName) {
		...
		String yesterday = "SELECT SUM(transaction_amount) yesterday from t_site_income WHERE create_time BETWEEN  (DATE_FORMAT(DATE_ADD(NOW(),INTERVAL -1 DAY),'%Y-%m-%d 00:00:00')) AND (DATE_FORMAT(DATE_ADD(NOW(),INTERVAL -1 DAY),'%Y-%m-%d 23:59:59')) AND portal_user_name not LIKE '0%'";
		String today = "SELECT SUM(transaction_amount) today from t_site_income WHERE create_time BETWEEN  (DATE_FORMAT(DATE_ADD(NOW(),INTERVAL 0 DAY),'%Y-%m-%d 00:00:00')) AND (DATE_FORMAT(DATE_ADD(NOW(),INTERVAL 0 DAY),'%Y-%m-%d 23:59:59')) AND portal_user_name not LIKE '0%'";
		String total = "SELECT SUM(transaction_amount) total from t_site_income where portal_user_name not LIKE '0%' ";
		if (siteId != -2) {
			...
			total += " and site_id in(select id from t_cloud_site where user_id = (select id from t_cloud_user where user_name=?))";
		}
		...
		try {
			...
			if (siteId != -2) {
				yesterdays = jdbcTemplate.queryForList(yesterday,
						new Object[] { siteId });
				todays = jdbcTemplate.queryForList(today,
						new Object[] { siteId });
				totals = jdbcTemplate.queryForList(total,
						new Object[] { siteId });
			} else {
				yesterdays = jdbcTemplate.queryForList(yesterday,
			...
			if (yesterdays.size() > 0 && yesterdays != null
					&& yesterdays.get(0).get("yesterday") != null) {
				map.put("yesterday", yesterdays.get(0).get("yesterday"));
			}
			if (todays.size() > 0 && todays != null
					&& todays.get(0).get("today") != null) {
				map.put("today", todays.get(0).get("today"));
			}
			if (totals.size() > 0 && totals != null
					&& totals.get(0).get("total") != null) {
	...
	public Map getTimeBetweenIncome(String startTime, String endTime,
			int siteId, String agentName) {
		String startTimes = startTime + " 00:00:00";
		String endTimes = endTime + " 23:59:59";
		String sql = "SELECT SUM(transaction_amount) income from t_site_income WHERE  portal_user_name not LIKE '0%' and create_time < ?";
		List<Map<String, Object>> list = null;
		Map map = new HashMap<>();
		if (startTime == null || "".equals(startTime)) {
			if (siteId != -2) {
				sql += " and site_id = ?";
				list = jdbcTemplate.queryForList(sql, new Object[] { endTimes,
						siteId });
			} else {
			...
			if (list.get(0).get("income") != null
					&& !"".equals(list.get(0).get("income"))) {
