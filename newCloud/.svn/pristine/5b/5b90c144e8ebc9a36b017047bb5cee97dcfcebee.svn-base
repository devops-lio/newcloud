...
public class DataStatisticsImpl{
	...
	/**
	 * @Description  获得缴费类型占比
	 * @date 2016年10月21日下午12:11:41
	 * @author guoyingjie
	 * @param siteId
	 * @return
	 */
	public List<Map<String, Object>> getTypeEvery(String siteId){
		String sqlSite = "SELECT COUNT(pay_name) finalCount,pay_name finalName FROM t_site_income where  site_id =? and pay_name <> '0' GROUP BY pay_name ";
		String sqlSiteAll = "SELECT SUM(s.num) totalCount FROM (SELECT COUNT(pay_name) num FROM t_site_income where  site_id = ? and pay_name <> '0' GROUP BY pay_name) s";
		List<Map<String, Object>> sitecount = null;
		List<Map<String, Object>> allcount = null;
		int sum = 0;
		try {
			List<Map<String, Object>> finalResult = new ArrayList();
			sitecount = jdbcTemplate.queryForList(sqlSite,new Object[] { siteId });
			allcount = jdbcTemplate.queryForList(sqlSiteAll,new Object[] { siteId });// 场所下所有缴费类型总数
			sum = Integer.valueOf(allcount.get(0).get("totalCount") + "");
