...
(function($) {
	...
	$.fn.goFaq = function(options) {
		...
		/**
		 * Iterate through the collection of elements and return the object to
			...
			$list.find('li').each(
					function(i) {
						...
						if (options.enableStyling) {
							var icon = '<div class="faq-icon">'
									+ options.numberHtml + '</div>';
		...
		function generateToc($list) {
			...
			$list.find('li').each(
					function(i) {
						var id = parseInt(i + 1);
						html += '<li>' + id + '. <a href="#faq-' + id + '">'
								+ $(this).find('h4').text() + '</a></li>';
						$(this).attr('id', 'faq-' + id);
					});
